...
def label_shift(n_target = 1000, n_source = 20000, pi_target = 0.1,\
    ...
    ### Reweighted
    sample_weight_0 = 1/(1-pi_source)
    sample_weight_1 = 1/(pi_source)
    ...
    model_label_shift = tf.keras.models.Sequential([
            tf.keras.layers.Conv1D(3, 2, input_shape=(28, 28)),
            tf.keras.layers.Flatten(),
            tf.keras.layers.Dense(128, activation='relu'),
            tf.keras.layers.Dense(20),
    ...
    model_label_shift.fit(x_source, y_source, epochs=epochs, sample_weight = sample_weight, batch_size=batch_size)
    ...
    model_gs.fit(x_target, y_target, epochs=epochs, batch_size=batch_size)
...
accuracy_pilot, accuracy_reweighted, accuracy_label_shift, accuracy_train_pilot,\
        accuracy_train_reweighted, accuracy_train_ls, accuracy_gs = label_shift(n_target=n_target,\
         pi_source=pi_source, pi_target=pi_target, epochs= 50, batch_size=250)
